...
const Posts: React.FC = () => {
  ...
  useEffect(() => {
    const getPosts = async () => {
      try {
        ...
        const blogPosts = postsData.filter((post: any) => post.section === "blog"); // Filter posts by section
        setPosts(blogPosts);
      } catch (error) {
  ...
  if (loading) {
    return <div className="min-h-screen flex items-center justify-center">Loading blog posts...</div>;
  }
    ...
    >
      <div className="amharic-text max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        {posts.length > 0 ? (
            ...
            <div key={post.id}>
              ...
              >
                ...
                <div className="md:w-2/3 flex-grow w-full">
                  <h2 className="text-3xl md:text-4xl font-bold mb-6 text-gray-900">{post.header}</h2>
          ...
          <p className="text-center text-gray-700">No blog posts available.</p>
        )}
      </div>
    </section>
  );
